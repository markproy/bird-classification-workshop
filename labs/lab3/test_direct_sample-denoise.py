...
#    print(msg)
    ...
    print('denoising...')
#    img = denoise_tv_chambolle(img_as_float64(img), weight=0.2, multichannel=True)
    cv2_clean_img = cv2.fastNlMeansDenoisingColored(cv2_img,None,10,10,7,21)
    cv2.imwrite('clean-cv.jpg', cv2_clean_img) #increase_brightness(cv2_clean_img, 100))
    print('done denoising.')

    sk_img = denoise_tv_chambolle(cv2_img, weight=0.1)
    cv2.imwrite('clean-sk.jpg', sk_img)
    print('done denoising with SK.')


    print('Classify CLEAN image...')
    filename = 'clean.jpg'
    f = open(filename, 'rb')
    img = f.read()
    msg, top_k = nabird_classify_image(img, epn)
